...
def mf_possible_portfolios(returns,n_port,fq_adj):
	#randomly generate n weights
	n_asset = len(returns.T)
	weights = []
	np.random.seed(123)
...
def	mf_efficient_frontier(returns,fq_adj):
	n_asset = len(returns.T)
	returns = np.asmatrix(returns.T)
	#Assume
	#returns of each asset is a column vector
...
port_risks, port_returns = mf_possible_portfolios(returns,1000,fq_adj)

ef_risks, ef_returns, ef_weights = mf_efficient_frontier(returns,fq_adj)
